...
export const transform = (content: any): any => {
  ...
  if (
    content.provider_id == 'pcex' ||
    content.provider_id == 'pcex_ch' ||
    content.provider_id == 'pcex_activity'
  ) {
    content.provider_name = 'PCEX';
  } else if (content.provider_id == 'parsons') {
    content.provider_name = 'jsParsons';
  } else if (
    content.provider_id == 'opendsa_problems' ||
    content.provider_id == 'opendsa_slideshows'
  ) {
  ...
  if (content.author_name == 'Administrator') {
    ...
    // administrator-quizjet with meaningful name is by Sharon Hsiao
      ...
      content.provider_id == 'quizjet' &&
      !content.name.startsWith('subproblem ')
    ) {
