...
def load_local_data(file_path):
    print(f"Loading legacy data from local file: {file_path} ...", file=sys.stderr)
    ...
    try:
        ...
        print("File is not standard JSON. Parsing as JavaScript object via Node...", file=sys.stderr)
        node_cmd = [
            "node",
            "-e",
            "const fs = require('fs'); const raw = fs.readFileSync(0, 'utf-8'); const obj = eval('(' + raw + ')'); process.stdout.write(JSON.stringify(obj));"
...
def main():
    ...
    if args.local_file:
        ...
        try:
            ...
            print(f"Warning: Failed to fetch from legacy API ({e}). Falling back to local file...", file=sys.stderr)
            script_dir = os.path.dirname(os.path.abspath(__file__))
            fallback_path = os.path.join(script_dir, "..", "..", "migrate-old_courses", "prev-courseauthoring.json")
            if not os.path.exists(fallback_path):
                fallback_path = os.path.join(script_dir, "GetData.json")
    ...
    if args.cid != "all":
        ...
        if target_cid not in prev_courses:
            print(f"Error: Course CID {target_cid} not found in legacy data!", file=sys.stderr)
            sys.exit(1)
        ...
        print(f"\nSuccessfully cloned legacy course CID {target_cid}!", file=sys.stderr)
        print(f"  Name:        {cloned['name']}", file=sys.stderr)
        print(f"  Code:        {cloned['code']}", file=sys.stderr)
        print(f"  CID:         {cloned['cid']}", file=sys.stderr)
        print(f"  Domain:      {cloned['domain']}", file=sys.stderr)
        print(f"  Institution: {cloned['institution']}", file=sys.stderr)
        print(f"  User Email:  {cloned['user_email']}", file=sys.stderr)
        print(f"  Units:       {len(cloned['units'])}", file=sys.stderr)
        print(f"  Resources:   {len(cloned['resources'])}", file=sys.stderr)
        act_count = sum(len(acts) for u in cloned["units"] for acts in u["activities"].values())
        print(f"  Activities:  {act_count}", file=sys.stderr)
    else:
        courses = []
        authors = {}
        ...
        print(f"Successfully migrated {len(courses)} courses to {out_file}", file=sys.stderr)
